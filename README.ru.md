...
Код, Phaser 3, стили, данные и изображение включены в один HTML. Офлайн работает даже при первом открытии. Без WebGL Phaser использует Canvas; игровая логика остаётся доступной. Текущая партия сохраняется в браузере, если его настройки разрешают; результат можно экспортировать в JSON.
...
- Изометрическая карта на Phaser 3: пять районов, шесть узнаваемых зданий Астаны, 28 машин и 44 пешехода, масштабирование, день/ночь и видимые изменения после решений.
- Вступление в должность с вымышленным советником Айдой и подсказкой для первого хода.
...
Бәйтерек, Ақорда, Хан Шатыр, мечеть Әзірет Сұлтан, Дворец мира и согласия и Нұр Әлем нарисованы как отдельные игровые спрайты. Машины и пешеходы движутся по улицам. Визуальные эффекты иллюстрируют решения, а расчёт остаётся воспроизводимым.
...
**React + TypeScript + Vite + Phaser 3.** Чистый модуль `src/game/engine.ts` валидирует решения и считает все числа. Интерфейс и карта читают его результат. База данных и CDN не нужны; сборка упаковывает приложение в один HTML.
