...
export default function App() {
  const [lang, setLang] = useState<Lang>(initialLang),
    ...
    [layer, setLayer] = useState<'city' | 'score'>('city'),
  ...
  const result = useMemo(() => simulate(decisions), [decisions]),
    district = DISTRICTS.find((d) => d.id === selected)!;
  ...
  const policies = MEASURES.filter((m) => filter === 'all' || m.category === filter);
  ...
  return (
    <>
      ...
      <main>
        ...
        {view === 'city' && (
          <>
            <div className="city-layout">
              <section className="map-panel">
                <div className="section-top">
                  <div>
                    <span className="eyebrow">
                      {t('ВАША АСТАНА', 'YOUR ASTANA', 'СІЗДІҢ АСТАНАҢЫЗ')}
                    </span>
                    <h2>
                      {t(
                        'Посмотрите на город ближе',
                        'See the bigger picture',
                        'Қалаға жақынырақ қараңыз',
                      )}
                    </h2>
                  </div>
                  <div className="segmented">
                    <button
                      className={layer === 'city' ? 'active' : ''}
                    ...
                    <button
                      className={layer === 'score' ? 'active' : ''}
                      onClick={() => setLayer('score')}
                    >
                      {t('Качество жизни', 'Quality of life', 'Өмір сапасы')}
                    </button>
                  </div>
                </div>
                <CityMap
                ...
                <div className="map-bottom">
                  <span>
                    ...
                    {t(
                      'Нажмите на район, чтобы узнать больше',
                      'Select a district to explore its needs',
                      'Қажеттіліктерін білу үшін ауданды таңдаңыз',
                    )}
                  </span>
                  <span>
                    {t(
                      'Вращайте · приближайте · исследуйте',
                      'Rotate · zoom · explore',
                      'Бұрыңыз · үлкейтіңіз · зерттеңіз',
                    )}
                  </span>
                </div>
              </section>
              <aside className="district-panel">
                <div className="district-heading">
                  <div>
                    <span className="eyebrow">
                      {t('В ФОКУСЕ', 'IN FOCUS', 'НАЗАРДА')} · {Math.round(district.pop * 100)}%{' '}
                      {t('ЖИТЕЛЕЙ', 'OF RESIDENTS', 'ТҰРҒЫН')}
                    </span>
                    <h2>
                      {district.name[lang]}
                  ...
                  <span
                    className={`district-score ${result.districtScores[selected] < 52 ? 'low' : ''}`}
                  >
                    {result.districtScores[selected].toFixed(1)}
                  </span>
                </div>
                <p className="district-profile">{district.profile[lang]}</p>
                <div className="district-metrics">
                  {Object.entries(CATEGORIES).map(([key, cat]) => {
                    const Icon = ICONS[key as Category],
                      ...
                      base =
                        cat.keys.reduce((s, k) => s + district.metrics[k] * WEIGHTS[k], 0) /
                        cat.keys.reduce((s, k) => s + WEIGHTS[k], 0);
                    return (
                      <div className="metric" key={key}>
                        <div>
                          ...
                          <b>
                            {value.toFixed(0)}
                            {value > base && <em> +{(value - base).toFixed(1)}</em>}
                          </b>
                        </div>
                        <div className="metric-track">
                          <i style={{ width: `${value}%`, background: cat.color }} />
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="district-note">
                  ...
                  <p>
                    {result.critical.some((c) => c.districtId === selected)
                      ? t(
                          'Есть показатели ниже 40. Району нужна ваша поддержка.',
                          'Some indicators are below 40. This district needs your support.',
                          'Кей көрсеткіштер 40-тан төмен. Ауданға көмек қажет.',
                        )
                      : t(
                          'Нет критических показателей. Но всегда можно сделать лучше.',
                          'No critical indicators. There is still room to improve.',
                          'Сындарлы көрсеткіштер жоқ. Жақсартуға мүмкіндік бар.',
                        )}
                  </p>
                </div>
                <button
                  ...
                  onClick={() => {
                    ...
                    catalog.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                  }}
                >
                  {t('Подобрать решение', 'Explore policies', 'Шешім таңдау')}
                  <ArrowRight size={17} />
                </button>
              </aside>
            </div>
            <section className="advisor-banner">
              ...
              <div>
                <strong>
                  {t('Советник города', 'City advisor', 'Қала кеңесшісі')}
                  <span className="tiny-tag">
                    {t('ЛОКАЛЬНЫЙ АНАЛИЗ', 'LOCAL ANALYSIS', 'ЖЕРГІЛІКТІ ТАЛДАУ')}
                  </span>
                </strong>
                <p>{localAdvice}</p>
              </div>
              {suggestion && (
                ...
                >
                  {t('Посмотреть совет', 'Explore suggestion', 'Кеңесті қарау')}
            ...
            <section className="catalog" ref={catalog}>
                ...
                <div>
                  <span className="eyebrow">
                    {t(
                      ...
                      'ШАҒЫН ҚАДАМДАР. ҮЛКЕН ӨЗГЕРІСТЕР.',
                    )}
                  </span>
                  <h2>
                    {complete
                      ? t(
                          'Ваши решения меняют город',
                          'Your choices shape the city',
                          'Шешімдеріңіз қаланы өзгертеді',
                        )
                      : t(
                          'Что сделаем для города?',
                          'What will you do for your city?',
                          'Қала үшін не істейміз?',
                        )}
                  </h2>
                </div>
                <p>
                  {t(
                    'Выберите карточку → оцените эффект → примите решение',
                    'Pick a card → preview the impact → make your move',
                    'Картаны таңдаңыз → әсерін бағалаңыз → шешім қабылдаңыз',
                  )}
                </p>
              </div>
              <div className="filters">
                ...
                >
                  {t('Все решения', 'All policies', 'Барлық шешімдер')}
                  <span>14</span>
                </button>
                {Object.entries(CATEGORIES).map(([key, cat]) => {
                  const Icon = ICONS[key as Category];
              ...
              <div className="policy-grid">
                {policies.map((m) => {
                  const Icon = ICONS[m.category],
                    adopted = decisions.some((d) => d.measureId === m.id);
                  return (
                    <article
                      className={`policy-card ${adopted ? 'adopted' : ''}`}
                      key={m.id}
                      style={
                        { '--category-color': CATEGORIES[m.category].color } as React.CSSProperties
                      }
                    >
                      <div className={`policy-art art-${m.category}`}>
                        <div className="art-lines" />
                        <Icon size={55} strokeWidth={1.1} />
                        <span className="policy-category">{CATEGORIES[m.category].name[lang]}</span>
                        <span className="cost-pill">
                          {m.cost}
                          <small>◈</small>
                        </span>
                        {SYNERGIES.some((s) => s.pair.some((id) => id === m.id)) && (
                          <span
                            className="combo-corner"
                            title={t('Есть синергия', 'Has a synergy', 'Синергия бар')}
                          >
                            <Link2 size={14} />
                          </span>
                        )}
                          ...
                          {Object.entries(m.effects)
                            ...
                            .map(([k, v]) => (
                              <span
                                ...
                                key={k}
                                title={INDICATORS[k as keyof typeof INDICATORS][lang]}
                              >
                                {k} {v > 0 ? '+' : ''}
                                {(v * (8 - m.lag)) / 8}
                              </span>
                            ))}
                          <small>{t('за 2 года', 'over 2 years', '2 жылда')}</small>
                        </div>
                        <button
                          data-testid={`policy-${m.id}`}
                          disabled={adopted || complete}
                          onClick={() => choose(m)}
                        >
                          {adopted ? (
                            <>
                            ...
                            <>
                              {t('Рассмотреть', 'Explore policy', 'Қарастыру')}
            ...
            {decisions.length > 0 && (
              <section className="decision-history">
                <div className="catalog-heading">
                  <h2>{t('Ваш план города', 'Your city plan', 'Сіздің қала жоспарыңыз')}</h2>
                  <button
                    className="text-button"
                    onClick={() => setDecisions(decisions.slice(0, -1))}
                  >
                    <Undo2 size={16} />
        ...
        {view === 'report' && (
          <section className="report">
            ...
            <div className="report-stats">
              <article>
                <small>{t('ГОРОД В ЦЕЛОМ · 70%', 'CITY AVERAGE · 70%', 'ЖАЛПЫ ҚАЛА · 70%')}</small>
                ...
                {DISTRICTS.map((d) => (
                  <div className="district-result" key={d.id}>
                    ...
                    <span>
                      ...
                      <b>{signed(result.districtScores[d.id] - BASELINE.districtScores[d.id])}</b>
                    </span>
                  </div>
                ))}
                <small>
                ...
                {location.protocol !== 'file:' && (
                  <button className="secondary" disabled={aiBusy} onClick={askAI}>
                    ...
                    {aiBusy
                      ...
                      : t('Запросить LLM-разбор', 'Request LLM explanation', 'LLM талдауын сұрау')}
                  </button>
                )}
                {aiText && (
              ...
              <div className="contribution-list">
                {contributions(decisions).map(({ decision: d, gain }) => (
                  <div key={d.measureId}>
                    ...
                    <strong className={gain < 0 ? 'negative' : 'positive'}>{signed(gain)}</strong>
                  </div>
                ))}
              </div>
            </section>
                  ...
                  <tbody>
                    {KEYS.map((k) => (
                      <tr key={k}>
                        ...
                        <td>{WEIGHTS[k]}</td>
                        {DISTRICTS.map((d) => (
                          <td key={d.id} className={result.metrics[d.id][k] < 40 ? 'negative' : ''}>
                      ...
                      <ul>
                        {run.decisions.map((d) => (
                          <li key={d.measureId}>
                            ...
                            {d.districtId ? districtName(d.districtId) : t('город', 'city', 'қала')}
                          </li>
                        ))}
                      </ul>
                      <small>
        ...
        <footer>
          <span>
            <b>QALA</b> · {t('Город — это люди.', 'A city is its people.', 'Қала — бұл адамдар.')}
          </span>
