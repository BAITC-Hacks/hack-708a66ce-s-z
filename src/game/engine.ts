...
export interface Result {
  metrics: Record<DistrictId, Metrics>;
  districtScores: Record<DistrictId, number>;
  average: number;
  weakest: DistrictId;
  critical: { districtId: DistrictId; indicator: Indicator; value: number }[];
  score: number;
  cost: number;
  synergies: (typeof SYNERGIES)[number][];
}
export function validate(decisions: readonly Decision[], complete = true): Issue[] {
  ...
  for (const [a, b, code] of [
    ...
    const first = decisions.find((d) => d?.measureId === a),
      second = decisions.find((d) => d?.measureId === b);
...
export function simulate(decisions: readonly Decision[]): Result {
  ...
  for (const d of [...decisions].sort((a, b) => a.measureId.localeCompare(b.measureId))) {
    ...
    for (const id of targets)
      for (const [k, v] of Object.entries(m.effects))
        metrics[id][k as Indicator] += (v * (8 - m.lag)) / 8;
  }
  const synergies = SYNERGIES.filter((s) =>
    s.pair.every((id) => decisions.some((d) => d.measureId === id)),
  );
  for (const s of synergies) {
    const target = decisions.find((d) => d.measureId === s.pair[0])!.districtId!;
    metrics[target][s.indicator] += s.bonus;
  }
  ...
  for (const d of DISTRICTS) {
    let score = 0;
    for (const k of KEYS) {
      metrics[d.id][k] = clip(metrics[d.id][k]);
      score += metrics[d.id][k] * WEIGHTS[k];
      if (metrics[d.id][k] < 40)
        critical.push({ districtId: d.id, indicator: k, value: metrics[d.id][k] });
    }
    districtScores[d.id] = score;
    average += d.pop * score;
  }
...
export function finalResult(decisions: readonly Decision[]): {
  issues: Issue[];
  result: Result | null;
} {
  const issues = validate(decisions);
  return { issues, result: issues.length ? null : simulate(decisions) };
}
export const BASELINE = simulate([]);
export function candidates(decisions: readonly Decision[]): Decision[] {
  return MEASURES.filter((m) => !decisions.some((d) => d.measureId === m.id))
    .flatMap((m) =>
      m.scope === 'city'
        ? [{ measureId: m.id }]
        : DISTRICTS.map((d) => ({ measureId: m.id, districtId: d.id })),
    )
    .filter((d) => !validate([...decisions, d], false).length);
}
...
export function canComplete(decisions: readonly Decision[]): boolean {
  ...
  const remaining = MEASURES.filter((m) => !decisions.some((d) => d.measureId === m.id)).sort(
    (a, b) => a.cost - b.cost,
  );
  const used = decisions.reduce((v, d) => v + MEASURES.find((m) => m.id === d.measureId)!.cost, 0);
  if (used + remaining.slice(0, 5 - decisions.length).reduce((v, m) => v + m.cost, 0) > 100)
    return false;
  for (const m of remaining)
    for (const target of m.scope === 'city' ? [undefined] : DISTRICTS.map((d) => d.id)) {
      if (
        canComplete([...decisions, { measureId: m.id, ...(target ? { districtId: target } : {}) }])
      )
        return true;
    }
  return false;
}
export function moveIssues(decisions: readonly Decision[], next: Decision): Issue[] {
  const issues = validate([...decisions, next], false);
  return issues.length ? issues : canComplete([...decisions, next]) ? [] : [{ code: 'dead-end' }];
}
export function recommend(
  decisions: readonly Decision[],
): { decision: Decision; gain: number } | null {
  const base = simulate(decisions).score;
  const ranked = candidates(decisions)
    .map((decision) => ({ decision, gain: simulate([...decisions, decision]).score - base }))
    .sort((a, b) => b.gain - a.gain);
  return ranked.find((r) => canComplete([...decisions, r.decision])) ?? null;
}
export function contributions(decisions: readonly Decision[]) {
  const total = simulate(decisions).score;
  return decisions.map((d) => ({
    decision: d,
    gain: total - simulate(decisions.filter((x) => x !== d)).score,
  }));
}
