...
// QALA: the canvas world and policy cards share one decoding/chroma-key pass.
...
export function loadSpriteImage(
  ...
  const existing = pendingSprites.get(cacheKey);
  if (existing) return existing;
  const pending = loadImage(src)
    .then(async (img) => {
      const result = applyFilter ? await filterBackgroundColor(img) : img;
      imageCache.set(cacheKey, result);
      return result;
    })
    .finally(() => pendingSprites.delete(cacheKey));
