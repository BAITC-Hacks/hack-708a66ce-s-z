 ...
 */
export function findRecreationAreas(grid: Tile[][], gridSize: number): RecreationDestination[] {
  if (!grid || gridSize <= 0) return [];
 ...
 */
export function findBusStops(grid: Tile[][], gridSize: number): { x: number; y: number }[] {
  if (!grid || gridSize <= 0) return [];
 ...
 */
export function findFires(grid: Tile[][], gridSize: number): { x: number; y: number }[] {
  if (!grid || gridSize <= 0) return [];
 ...
 */
export function findAirports(grid: Tile[][], gridSize: number): { x: number; y: number }[] {
  if (!grid || gridSize <= 0) return [];
 ...
 */
export function findHeliports(grid: Tile[][], gridSize: number): HeliportInfo[] {
  if (!grid || gridSize <= 0) return [];
 ...
 */
export function findMarinasAndPiers(grid: Tile[][], gridSize: number): DockInfo[] {
  if (!grid || gridSize <= 0) return [];
 ...
 */
export function findOceanConnectedMarinas(grid: Tile[][], gridSize: number): DockInfo[] {
  if (!grid || gridSize <= 0) return [];
 ...
 */
export function findOceanSpawnPoints(grid: Tile[][], gridSize: number): OceanSpawnPoint[] {
  if (!grid || gridSize <= 0) return [];
...
export function findConnectedWaterTiles(
  ...
  const directions = [
    ...
    [0, -1],
    [0, 1],
  ]; // 4-directional for cleaner water bodies
 ...
 */
export function findBays(grid: Tile[][], gridSize: number, minSize: number = 15): BayInfo[] {
  if (!grid || gridSize <= 0) return [];
 ...
 */
export function getRandomBayTile(bay: BayInfo): {
  x: number;
  y: number;
  screenX: number;
  screenY: number;
 ...
 */
export function calculateTotalPopulation(grid: Tile[][], gridSize: number): number {
 ...
 */
export function countRoadTiles(grid: Tile[][], gridSize: number): number {
  if (!grid || gridSize <= 0) return 0;
