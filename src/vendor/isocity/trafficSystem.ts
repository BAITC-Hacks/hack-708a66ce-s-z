...
export const ROAD_CONFIG = {
  ...
  AVENUE_LANE_WIDTH: 0.1, // Lanes for avenues
...
export function analyzeMergedRoad(
  ...
  if (isNSRoad && !isEWRoad) {
    ...
    const hasParallelWest =
      isRoad(grid, gridSize, x, y + 1) &&
      (isRoad(grid, gridSize, x - 1, y + 1) || isRoad(grid, gridSize, x + 1, y + 1));
    const hasParallelEast =
      isRoad(grid, gridSize, x, y - 1) &&
      (isRoad(grid, gridSize, x - 1, y - 1) || isRoad(grid, gridSize, x + 1, y - 1));
  ...
  if (isEWRoad && !isNSRoad) {
    ...
    const hasParallelNorth =
      isRoad(grid, gridSize, x - 1, y) &&
      (isRoad(grid, gridSize, x - 1, y - 1) || isRoad(grid, gridSize, x - 1, y + 1));
    const hasParallelSouth =
      isRoad(grid, gridSize, x + 1, y) &&
      (isRoad(grid, gridSize, x + 1, y - 1) || isRoad(grid, gridSize, x + 1, y + 1));
...
export function getTrafficLightState(time: number): TrafficLightState {
  ...
  if (cycleTime < TRAFFIC_LIGHT_TIMING.GREEN_DURATION) {
    ...
    cycleTime <
    TRAFFIC_LIGHT_TIMING.GREEN_DURATION + TRAFFIC_LIGHT_TIMING.YELLOW_DURATION
  ) {
    return 'yellow_ns'; // North-South yellow
  } else if (
    cycleTime <
    TRAFFIC_LIGHT_TIMING.GREEN_DURATION * 2 + TRAFFIC_LIGHT_TIMING.YELLOW_DURATION
  ) {
 ...
 */
export function getTrafficFlowDirection(mergeInfo: MergedRoadInfo): CarDirection[] {
...
export function drawMergedRoadSegment(
  ...
  const laneWidth =
    mergeInfo.type === 'highway'
      ? ROAD_CONFIG.HIGHWAY_LANE_WIDTH
      : mergeInfo.type === 'avenue'
        ? ROAD_CONFIG.AVENUE_LANE_WIDTH
  ...
  const drawSidewalks =
    mergeInfo.side === 'left' || mergeInfo.side === 'right' || mergeInfo.side === 'single';
...
export function drawCrosswalks(params: CrosswalkParams): void {
  ...
  const drawCrosswalk = (
    ...
    edgeY: number,
    dirDx: number,
    dirDy: number, // Direction toward edge (traffic flow)
    perpDx: number,
    perpDy: number, // Perpendicular (across road)
  ) => {
    // Center of crosswalk
