...
export type CarriageType =
  'locomotive' | 'passenger' | 'freight_box' | 'freight_tank' | 'freight_flat' | 'caboose';
...
function drawBallast(
  ...
  const drawSingleCurvedBallast = (
    ...
    ctx.quadraticCurveTo(
      control.x + normMidPerp.x * halfW,
      control.y + normMidPerp.y * halfW,
      to.x + toPerp.x * halfW,
      to.y + toPerp.y * halfW,
    );
    ...
    ctx.quadraticCurveTo(
      control.x - normMidPerp.x * halfW,
      control.y - normMidPerp.y * halfW,
      from.x - fromPerp.x * halfW,
      from.y - fromPerp.y * halfW,
    );
    ctx.closePath();
    ctx.fill();
  };
...
function drawTies(
  ...
  const drawTie = (
    ...
    ctx.beginPath();
    ctx.moveTo(
      tieX + tieDir.x * halfLen + tiePerpDir.x * halfWidth,
      tieY + tieDir.y * halfLen + tiePerpDir.y * halfWidth,
    );
    ctx.lineTo(
      tieX + tieDir.x * halfLen - tiePerpDir.x * halfWidth,
      tieY + tieDir.y * halfLen - tiePerpDir.y * halfWidth,
  ...
  const drawSingleCurveTies = (
    ...
    for (let i = 0; i < numTies; i++) {
      ...
      const interpPerpDir = {
        x: fromTiePerpDir.x * u + toTiePerpDir.x * t,
        y: fromTiePerpDir.y * u + toTiePerpDir.y * t,
      };
      const interpPerpLen = Math.hypot(interpPerpDir.x, interpPerpDir.y);
      const normTiePerpDir = {
        x: interpPerpDir.x / interpPerpLen,
        y: interpPerpDir.y / interpPerpLen,
      };
...
function drawInsetRails(
  ...
  switch (trackType) {
    ...
    case 'curve_se':
      ...
      break;
    case 'curve_sw':
      drawDoubleCurvedInsetRails(southEdge, westEdge, center, NEG_ISO_EW, NEG_ISO_NS, {
        x: 0,
        y: -1,
      });
      break;
    ...
    case 'junction_t_e':
      ...
      drawDoubleCurvedInsetRails(westEdge, southEdge, center, NEG_ISO_NS, NEG_ISO_EW, {
        x: 0,
        y: -1,
      });
      break;
 ...
 */
export function isRailroadCrossing(
  grid: Tile[][],
  gridSize: number,
  x: number,
  y: number,
 ...
 */
export function findRailroadCrossings(
  grid: Tile[][],
  gridSize: number,
): { x: number; y: number }[] {
...
export function drawCrossingGate(
  ...
  const offsets = {
    nw: { x: w * 0.15, y: h * 0.5, dir: 1, tilt: -30 }, // default swings right, tilts up
    ...
    se: { x: w * 0.85, y: h * 0.5, dir: -1, tilt: 30 }, // default swings left, tilts down
  };
...
export function findPathOnRails(
  ...
  while (queue.length > 0) {
    ...
    for (const { dx, dy } of directions) {
      ...
      if (!isRailTile(grid, gridSize, nx, ny) && !isRailStationTile(grid, gridSize, nx, ny))
        continue;
