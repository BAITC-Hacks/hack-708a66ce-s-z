...
export function drawPedestrians(
  ...
  if (filterMode === 'recreation') {
    ...
    visiblePedestrians = visiblePedestrians.filter(
      (ped) =>
        ped.state === 'at_recreation' ||
        ped.state === 'at_beach' ||
        ped.state === 'approaching_shop' ||
        (ped.activity === 'shopping' &&
          (ped.state === 'entering_building' || ped.state === 'exiting_building')),
    );
  ...
  for (let i = 0; i < visiblePedestrians.length; i++) {
    ...
    if (ped.state === 'at_recreation') {
      ...
      pedX =
        centerX +
        meta.vec.dx * ped.progress +
        meta.normal.nx * sidewalkOffset +
        ped.activityOffsetX;
      pedY =
        centerY +
        meta.vec.dy * ped.progress +
        meta.normal.ny * sidewalkOffset +
        ped.activityOffsetY +
        yOffset;
...
function drawBasketballPlayer(ctx: CanvasRenderingContext2D, ped: Pedestrian): void {
  ...
  ctx.arc(
    (4 + armMove * 0.3) * scale,
    (4 + Math.abs(armMove) + bounce) * scale,
    2 * scale,
    0,
    Math.PI * 2,
  );
...
function drawJogger(ctx: CanvasRenderingContext2D, ped: Pedestrian): void {
  const scale = 0.2; // Small scale for park joggers
...
function drawPlaygroundKid(ctx: CanvasRenderingContext2D, ped: Pedestrian): void {
  const scale = 0.2; // Small - it's a kid on playground
...
function drawShopperQueuing(ctx: CanvasRenderingContext2D, ped: Pedestrian): void {
  ...
  // Hair for variety
  if (ped.id % 2 === 0) {
    const hairColor = ['#2c1810', '#4a3728', '#8b4513', '#d4a574', '#f5deb3', '#1a1a1a'][
      ped.id % 6
    ];
    ctx.fillStyle = hairColor;
    ctx.beginPath();
