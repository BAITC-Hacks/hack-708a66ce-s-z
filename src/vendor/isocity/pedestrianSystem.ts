...
import { Tile, BuildingType } from './model';
import {
  Pedestrian,
  PedestrianState,
  PedestrianActivity,
...
const SHOP_BUILDINGS: BuildingType[] = ['shop_small', 'shop_medium', 'mall'];
...
export function createPedestrian(
  ...
  const hasBag =
    (destType === 'commercial' || destType === 'industrial') &&
    Math.random() < PEDESTRIAN_BAG_CHANCE;
  ...
  return {
    ...
    hasHat,
    hatColor: hasHat
      ? PEDESTRIAN_HAT_COLORS[Math.floor(Math.random() * PEDESTRIAN_HAT_COLORS.length)]
      : '#000000',
...
function updateBeachState(
  ...
  if (ped.activity === 'beach_swimming') {
    ...
    const swimRadius = 6;
    ped.activityOffsetX +=
      (Math.sin(ped.activityAnimTimer * 0.3) * 0.1 - ped.activityOffsetX * 0.02) * delta * 10;
    ped.activityOffsetY +=
      (Math.cos(ped.activityAnimTimer * 0.2) * 0.05 - ped.activityOffsetY * 0.02) * delta * 10;
    // Clamp to stay in reasonable area
    ped.activityOffsetX = Math.max(-swimRadius, Math.min(swimRadius, ped.activityOffsetX));
    ped.activityOffsetY = Math.max(
      -swimRadius * 0.5,
      Math.min(swimRadius * 0.5, ped.activityOffsetY),
    );
...
export function spawnPedestrianApproachingShop(
  ...
  const ped = createPedestrian(id, homeX, homeY, shopX, shopY, 'commercial', path, 0, 'south');
...
export function spawnPedestrianAtRecreation(
  ...
  const ped = createPedestrian(id, homeX, homeY, areaX, areaY, 'park', path, 0, 'south');
 ...
 */
export function findBeachTiles(grid: Tile[][], gridSize: number): BeachTileInfo[] {
  ...
  for (let y = 0; y < gridSize; y++) {
    for (let x = 0; x < gridSize; x++) {
      ...
      for (const check of adjacentChecks) {
        ...
        if (adjTile.building.type !== 'water' && !marinaPierTypes.includes(adjTile.building.type)) {
          beachTiles.push({
            waterX: x,
            waterY: y,
            landX: adjX,
