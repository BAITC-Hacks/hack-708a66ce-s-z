...
Public local HTML now accepts navigation from an offline file, a different local port, or an external link. Real HTTP checks return 200 for those document requests and 403 for cross-origin API requests or a foreign Host. The shell retains loopback-only access and forbids framing; the strict API guard is unchanged. Regression coverage verifies both `/` and `/index.html`, including credential and advice endpoints.
