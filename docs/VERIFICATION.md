...
The final source passes `npm run check` (52 tests, TypeScript and build). All 14 browser journeys pass: the main 13-test suite, the focused clipping/synergy regression, and refreshed Russian analytics/search screenshot scenarios. `npm run format:check` passes. The packaged `play/QALA.html` is regenerated from the same tested `dist/index.html`.
